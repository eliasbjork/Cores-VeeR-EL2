...
#(
    ...
    input logic         dec_fpu_div_cancel,
    ...
    output logic        exu_fpu_div_wren,
    output logic [31:0] exu_fpu_div_result
);
  ...
  logic fpu_valid_x, fpu_valid_x_in;
  logic [31:0] fpu_result_x, fpu_direct_result;
  logic fpu_direct_result_valid;
  ...
  rvdffie #(pt.BHT_GHR_SIZE + 3, 1) i_misc_ff (
      .*,
      .clk (clk),
      .din ({ghr_d_ns[pt.BHT_GHR_SIZE-1:0], mul_p.valid, fpu_valid_x_in, dec_i0_branch_d}),
      .dout({ghr_d[pt.BHT_GHR_SIZE-1:0], mul_valid_x, fpu_valid_x, i0_branch_x})
  );
  ...
  el2_exu_fpu_ctl #(
      ...
      .*,
      .fpu_p(fpu_p),
      .cancel(dec_fpu_div_cancel),
      .rs1(muldiv_rs1_d[31:0]),
      .rs2(i0_rs2_d[31:0]),
      .rs3(i0_rs3_d[31:0]),
      .x_result(fpu_result_x),
      .d_result(fpu_direct_result),
      .div_wren(exu_fpu_div_wren),
      .d_result_valid(fpu_direct_result_valid)
  );

  assign exu_fpu_div_result = fpu_direct_result;

  assign exu_i0_result_x[31:0]    =  (mul_valid_x)  ?  mul_result_x[31:0]  :
                                     (fpu_valid_x)  ?  fpu_result_x[31:0]  :
                                                       alu_result_x[31:0]  ;
